...
import { FavoriteSVGProps, Item } from "../types";
import { HighlightedCntxt } from "../contextAPI";
import { useRemoveFavorite } from "../Services/removeFavorite";

export function FavoriteSVG({selection}: FavoriteSVGProps) {

    const {favorites,setFavorites}= useContext(HighlightedCntxt)
    const alreadyAdded = favorites.some(elmnt => elmnt.id === selection?.id)

    const remove = useRemoveFavorite()

    function handleClick(selection: Item) {
        if (!selection) return
    ...
    return(
        ...
        fill={ alreadyAdded ? "red" : "currentColor"}
