...
export function extendHighlightStart({ matchedOpeningSpan, spanOpenTag }: extendHighlightStartProps) {
  ...
  const newHighlight = sameColor ? `${spanOpenTag}${noOpeningTag}` : `${spanOpenTag}${noOpeningTag}${spanCloseTag}`
  ...
  const newHtml = sameColor ? firstPart + newHighlight + lastPart : firstPart + newHighlight + presentSpan + lastPart
