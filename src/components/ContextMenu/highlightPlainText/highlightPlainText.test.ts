...
it('should highlight a plain text', () => {
  ...
  const highlightedText =
    'This function should be called when <span class="contextMenu_color--first">the user selects</span> a <span class="contextMenu_color--first">text with no highlight</span>';
...
it('should highlight an entire plain text when it is selected by triple-clicking', () => {
  jest.spyOn(window, 'getSelection').mockImplementation(
    () =>
      ({
        toString: () => 'This text was selected by triple clicking on it, because it has 2 additional blank spaces at the end in the selection  ',
        getRangeAt: () => ({
          startOffset: 0,
          startContainer: {
            previousSibling: {},
  ...
  const htmlContent = 'This text was selected by triple clicking on it, because it has 2 additional blank spaces at the end in the selection';
  const returnedHtml = highlightPlainText({ htmlContent, spanOpenTag });

  const highlightedText =
    '<span class="contextMenu_color--first">This text was selected by triple clicking on it, because it has 2 additional blank spaces at the end in the selection</span>';
  expect(returnedHtml).toBe(highlightedText);
});
...
it('should highlight a specific word in plain text, even if the word is repeated', () => {
  ...
  const highlightedText =
    'This <span class="contextMenu_color--second">string</span> has some repeated words such as "<span class="contextMenu_color--first">string</span>".';
...
it('should highlight a word even if it is included in the highlight tag', () => {
  ...
  const highlightedText =
    'The <span class="contextMenu_color--first">highlight tag</span> contains some words, such as context, <span class="contextMenu_color--first">color</span>, first, second...';
